...
{
	_write_rfreg( pAdapter, RF_PATH_A , RF_T_METER_8723B , BIT17 |BIT16 , 0x03 );
...
u8 Hal_ReadRFThermalMeter(PADAPTER pAdapter)
{
	...
	ThermalValue = _read_rfreg(pAdapter, RF_PATH_A, RF_T_METER_8723B, 0xfc00);
...
void Hal_SetSingleCarrierTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart)// Start Single Carrier.
	{
		...
		if(!read_bbreg(pAdapter, rFPGA0_RFMOD, bOFDMEn))
			write_bbreg(pAdapter, rFPGA0_RFMOD, bOFDMEn, bb_bEnable);//set OFDM block on

		{
		// 2. set CCK test mode off, set to CCK normal mode
		write_bbreg(pAdapter, rCCK0_System, bCCKBBMode, bb_bDisable);
		// 3. turn on scramble setting
		write_bbreg(pAdapter, rCCK0_System, bCCKScramble, bb_bEnable);
         }
		// 4. Turn On Single Carrier Tx and turn off the other test modes.
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMContinueTx, bb_bDisable);
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMSingleCarrier, bb_bEnable);
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMSingleTone, bb_bDisable);
	...
	else// Stop Single Carrier.
	{
		...
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMContinueTx, bb_bDisable);
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMSingleCarrier, bb_bDisable);
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMSingleTone, bb_bDisable);
#ifdef CONFIG_RTL8192C
		...
		if (pAdapter->mppriv.rateidx <= MPT_RATE_11M)
		  {
			...
			if(!read_bbreg(pAdapter, rFPGA0_RFMOD, bCCKEn))
				write_bbreg(pAdapter, rFPGA0_RFMOD, bCCKEn, bb_bEnable);//set CCK block on
...
void Hal_SetCCKContinuousTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart)
	{
		...
		write_bbreg(pAdapter, rCCK0_System, bCCKBBMode, 0x2);	//transmit mode
		write_bbreg(pAdapter, rCCK0_System, bCCKScramble, bb_bEnable);	//turn on scramble setting
		...
		if (cckrate == MPT_RATE_1M)
			write_bbreg(pAdapter, 0xA71, BIT(6), bb_bDisable);
		else
			write_bbreg(pAdapter, 0xA71, BIT(6), bb_bEnable);
#endif
...
void Hal_SetOFDMContinuousTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart) {
		...
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMContinueTx, bb_bEnable);
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMSingleCarrier, bb_bDisable);
		write_bbreg(pAdapter, rOFDM1_LSTF, bOFDMSingleTone, bb_bDisable);
