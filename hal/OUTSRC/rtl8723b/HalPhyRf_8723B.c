	...
	)
{
	...
	else if(bIQKOK)
	{
		...
		pRFCalibrateInfo->TxIQC_8723B[PATH_S1][IDX_0xC9C][KEY] = rOFDM0_XCTxAFE;
		pRFCalibrateInfo->TxIQC_8723B[PATH_S1][IDX_0xC9C][VAL] = ODM_GetBBReg(pDM_Odm, rOFDM0_XCTxAFE, bMaskDWord);

		ODM_SetBBReg(pDM_Odm, rOFDM0_XATxIQImbalance, 0x003F0000, (TX0_C&0x3F));
		pRFCalibrateInfo->TxIQC_8723B[PATH_S1][IDX_0xC88][KEY] = rOFDM0_XATxIQImbalance;
		pRFCalibrateInfo->TxIQC_8723B[PATH_S1][IDX_0xC88][VAL] = ODM_GetBBReg(pDM_Odm, rOFDM0_XATxIQImbalance, bMaskDWord);
	...
	)
{
	...
	else if(bIQKOK)
	{
		...
		pRFCalibrateInfo->TxIQC_8723B[PATH_S0][IDX_0xC94][KEY] = rOFDM0_XDTxAFE;
		...
		pRFCalibrateInfo->TxIQC_8723B[PATH_S0][IDX_0xC80][KEY] = rOFDM0_XBTxIQImbalance;
		...
		if(bTxOnly) {
			...
			pRFCalibrateInfo->RxIQC_8723B[PATH_S0][IDX_0xCA0][VAL] = ODM_GetBBReg(pDM_Odm, rOFDM0_AGCRSSITable, bMaskDWord);
			return;
		}
	...
	)
{
	...
	if(IS_2T2R( pHalData->VersionID)){
		return phy_QueryRFPathSwitch_8723B(pAdapter, TRUE);
	}
	else
#endif
	{
