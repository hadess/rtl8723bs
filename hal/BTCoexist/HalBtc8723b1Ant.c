...
u4Byte	GLCoexVerDate8723b1Ant=20130724;
u4Byte	GLCoexVer8723b1Ant=0x3c3a;
	...
	)
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], High Priority Tx/Rx (reg 0x%x)=%lx(%d)/%lx(%d)\n",
		regHPTxRx, regHPTx, regHPTx, regHPRx, regHPRx));
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], Low Priority Tx/Rx (reg 0x%x)=%lx(%d)/%lx(%d)\n",
		regLPTxRx, regLPTx, regLPTx, regLPRx, regLPRx));
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], WiFi RSSI = %d / BT RSSI = %d\n",
		wifiRssi, pCoexSta->btRssi));

	u4Tmp = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x778);
	u4Tmp1 = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x948);

	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], 0x778 = 0x%x / 0x948 = 0x%x\n",
		u4Tmp, u4Tmp1));

	u1Tmp = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x92c);
	u1Tmp = u1Tmp & 0x3;
	u1Tmp1 = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x930);

	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_BT_MONITOR, ("[BTCoex], 0x92c[1:0] = 0x%x / 0x930[7:0] = 0x%x\n",
		u1Tmp, u1Tmp1));
#endif
}
	...
	)
{
	...
	BOOLEAN			bChangePsPollMode=FALSE;
	...
	if(byte1 & BIT0) {
		if(bApEnable) {
			if ((BT_8723B_1ANT_BT_STATUS_CONNECTED_IDLE != pCoexDm->btStatus)  &&
			...
			pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_CHANGE_PSPOLL, &bChangePsPollMode);
			if (bChangePsPollMode) {
				if(!(realByte1 & BIT5)) { //NoPS = 0, psmode open
					BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], change pspoll mode\n"));
					realByte1 &= ~BIT4;//AutoWakeUp = 0, close null
					realByte1 |= BIT2;//TxPspollIntheMedium = 1, open pspoll
				}
			}
		}
	}
...
VOID halbtc8723b1ant_SetPsScan(
		IN	PBTC_COEXIST	pBtCoexist,
		IN	u1Byte		scanType
		)
{
...
VOID
halbtc8723b1ant_SetAntPos(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bAntforWifi
	)
{
	...
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_U4_WIFI_OUT_DPDT, &OutDpdt);
	if (fwVer < 0xc00 || OutDpdt)  //internal switch for WiFi fw ver >= 0xc
	{
		if(pBoardInfo->btdmAntPos == BTC_ANTENNA_AT_MAIN_PORT)
		{
			pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x92c, 0x3, bAntforWifi?1:2);
		}
		else
		{
			pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x92c, 0x3, bAntforWifi?2:1);
		}
	}
	else
	{
		if(pCoexDm->curPsTdma != 8)//PTA not this
		{
			if(pBoardInfo->btdmAntPos == BTC_ANTENNA_AT_MAIN_PORT)
			{
				pBtCoexist->fBtcWrite2Byte(pBtCoexist, 0x948, bAntforWifi?0:0x280);
			}
			else
			{
				pBtCoexist->fBtcWrite2Byte(pBtCoexist, 0x948, bAntforWifi?0x280:0);
			}
		}
	}
}

void halbtc8723b1ant_opp_Cmcc(
		IN	PBTC_COEXIST		pBtCoexist,
		IN	u8		bTurnOn,
		IN	u8		type
		)
{
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL, ("[BTCoex] halbtc8723b1ant_opp_Cmcc\n"));
	...
	)
{
	...
	if(!bForceExec)
	{
		...
		/* because turn off TDMA and then leave lps will change */
		/* to PAT, if we dont turn off TDMA again, PTA will */
		/* happen under wrong condotion */
	...
	if(bTurnOn)
	{
		/* EN_BCN_FUNCTION because TDMA and power save all need it */
		u1Byte	u1Tmp = 0;
		u1Tmp = pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x550) | BIT(3);
		pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x550, u1Tmp);

		switch(type)
		{
			case 1:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x53, 0x2c, 0x03, 0x10, 0x50);
				rssiAdjustVal = 11;
				break;
			case 2:
				if (drv_in_test == CMCC_TP_TEST)
					halbtc8723b1ant_opp_Cmcc(pBtCoexist, bTurnOn, type);
				else
					halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x53, 0x2b, 0x03, 0x10, 0x50);
			...
			case 3:
				if (drv_in_test == CMCC_TP_TEST)
					...
					halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x51, 0x1d, 0x1d, 0x00, 0x10);
				break;
			case 4:
				...
				break;
			case 5:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x61, 0x15, 0x03, 0x11, 0x10); //from 0x61 to 0x69 , ant for wifi when scan.by xx
				break;
			case 6:
			...
			case 8:
				...
				break;
			case 9:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x53, 0x21, 0x03, 0x10, 0x50);
				rssiAdjustVal = 18;
				break;
			case 10:
			...
			case 11:
				if (drv_in_test == CMCC_TP_TEST)
					...
					halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x53, 0x15, 0x03, 0x10, 0x10);
				rssiAdjustVal = 20;
				break;
			case 12:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xeb, 0xa, 0x3, 0x31, 0x18);
				break;
			case 13:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x51, 0x18, 0x18, 0x0, 0x10);
				break;
			case 15:
			...
			case 20:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x13, 0x25, 0x25, 0x0, 0x0);
				break;
			case 21:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x93, 0x20, 0x3, 0x10, 0x40);
				break;
			case 22:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x13, 0x8, 0x8, 0x0, 0x40);
				break;
			case 23:
			...
			case 28:
				...
				break;
			case 29:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xa3, 0x1a, 0x1a, 0x1, 0x10);
				break;
			case 30:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x93, 0x0a, 0x3, 0x14, 0x0);
				break;
			case 31:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xd3, 0x1a, 0x1a, 0, 0x58);
				break;
			case 32:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x63, 0xa, 0x3, 0x10, 0x00);
				break;
			case 33:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xa3, 0x25, 0x3, 0x30, 0x90);
				break;
			case 34:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xd3, 0x1a, 0x1a, 0x0, 0x10);
				break;
			case 35:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xe3, 0x1a, 0x1a, 0x0, 0x10);
				break;
			case 36:
			...
			case 40: // SoftAP only with no sta associated,BT disable ,TDMA mode for power saving
				...
				break;
			default:
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0xd3, 0x1a, 0x1a, 0x0, 0x50);
				break;
		}
	}
	else
	{
		...
		switch(type)
		{
			...
			case 9:
				...
				//pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x92c, 0x1);// 1 for WiFi
				halbtc8723b1ant_SetAntPos(pBtCoexist,TRUE);
				break;
			case 0:
			default:
				//ant to bt
				halbtc8723b1ant_SetFwPstdma(pBtCoexist, 0x0, 0x0, 0x0, 0x0, 0x0);
				//pBtCoexist->fBtcWrite1Byte(pBtCoexist, 0x92c, 0x2);
				halbtc8723b1ant_SetAntPos(pBtCoexist,FALSE);
				break;
		}
	}
...
VOID halbtc8723b1ant_CloseTdmaforSoftAP(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	u8	count = 0;
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], halbtc8723b1ant_CloseTdmaforSoftAP\n"));

	/* 8723bs new fw dont need it */
#if 0
	/* wait RF OFF slot */
	for (count =0; count<50; count++) {
		value8 = pBtCoexist->fBtcRead1Byte(pBtCoexist,0x02);
		//value8 = rtw_read8(padapter, 0x02);
		if ((value8 & 0x01) == 0 ) {
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave: RF have OFF\n"));
			break;
		} else {
			rtw_msleep_os(2);
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave: wait RF OFF count %d  \n", count));
		}
	}
	...
	for (count =0; count<50; count++) {
		value8 = pBtCoexist->fBtcRead1Byte(pBtCoexist,0x02);
		if ((value8 & 0x01) != 0) {
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave: sta case\n"));
			halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, FALSE, 9);
			halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 0);
			break;
		} else {
			rtw_msleep_os(2);
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave: wait RF ON count %d  \n", count));
		}
	}
#else
	halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, FALSE, 9);
...
{
	u1Byte	bWifiBusy=FALSE, bSoftAPIdle=TRUE;
	u1Byte	busy_traffic = 0;
	u1Byte	blinking = 0,bInEarlySp=0;

	if (type == 1) {
		/* close TDMA when RF is ON */
		halbtc8723b1ant_CloseTdmaforSoftAP(pBtCoexist);
		return;
	}

	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_BUSY, &bWifiBusy);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_SOFTAP_IDLE, &bSoftAPIdle);

	/* set busy_traffic when softap connected & busy */
	//if ((!bSoftAPIdle) && bWifiBusy)
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], BTDM_SoftAP_PowerSave  in_early_suspend:%d busy:%d linking:%d,bSoftAPIdle:%d\n",
				bInEarlySp, busy_traffic,blinking,bSoftAPIdle));

	/* broadcom always power save under softap, but we just power save under early_suspend */
	/* we don't care linked or no linked */
	if (bInEarlySp && !busy_traffic && !blinking) {
		if(pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x200) ==pBtCoexist->fBtcRead1Byte(pBtCoexist, 0x204)) {
...
//=============================================
//
//	Non-Software Coex Mechanism matrix start
//
//=============================================
...
{
	halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_OFF, 0x0, 0x0);
	...
	)
{
	...
	{
		halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_ON, 0x0, 0x0);
	...
	)
{
	...
	if (BT_8723B_1ANT_WIFI_STATUS_NON_CONNECTED_ASSO_AUTH_SCAN == wifiStatus) {
		halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_OFF, 0x0, 0x0);
		halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, TRUE, 29);
		halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 1);
		return;
	}

	if (BT_8723B_1ANT_WIFI_STATUS_CONNECTED_SCAN == wifiStatus) {
		if(pCoexDm->curPsTdma != 1 && pCoexDm->curPsTdma != 2 && pCoexDm->curPsTdma != 3)
		{
			halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_ON, 0x0, 0x0);
			halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, TRUE, 9);
			halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 1);
	...
	if (BT_8723B_1ANT_WIFI_STATUS_CONNECTED_SPECIAL_PKT == wifiStatus) {
		pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_DRIVER_TEST_CFG, &drv_in_test);
		if (drv_in_test == CMCC_TP_TEST) {
			halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_ON, 0x0, 0x0);
			halbtc8723b1ant_opp_Cmcc(pBtCoexist, TRUE, 2);
			halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 1);
		} else {
			if (!pCoexDm->bCurPsTdmaOn) {
				halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_ON, 0x0, 0x0);
				halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, TRUE, 1);
				halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 1);
			} else if(pCoexDm->curPsTdma != 1 && pCoexDm->curPsTdma != 2 && pCoexDm->curPsTdma != 3) {
				halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_ON, 0x0, 0x0);
				halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, TRUE, 9);
				halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 1);
			}
	...
	if( (pBtLinkInfo->bA2dpOnly) || (pBtLinkInfo->bHidExist&&pBtLinkInfo->bA2dpExist) )  //A2DP, HID+A2DP
	{
		if( (btRssiState == BTC_RSSI_STATE_HIGH) ||
			(btRssiState == BTC_RSSI_STATE_STAY_HIGH) )
		{
...
VOID
halbtc8723b1ant_ActionBtConnectedOrNoConnectedIdle(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	u1Byte				wifiStatus
	)
{
...
//=============================================
//
//	Non-Software Coex Mechanism matrix end
//
//=============================================
	...
	)
{
	...
	else if(bHsConnecting)
	{
		halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_OFF, 0x0, 0x0);
		halbtc8723b1ant_ActionHs(pBtCoexist);
	}
	else if( (BT_8723B_1ANT_BT_STATUS_NON_CONNECTED_IDLE == pCoexDm->btStatus) ||
		(BT_8723B_1ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus) )
	{
		halbtc8723b1ant_ActionBtConnectedOrNoConnectedIdle(pBtCoexist,
			BT_8723B_1ANT_WIFI_STATUS_NON_CONNECTED_ASSO_AUTH_SCAN);
	}
	else if(BT_8723B_1ANT_BT_STATUS_ACL_BUSY == pCoexDm->btStatus)
	{
	...
	)
{
	...
	if( (BT_8723B_1ANT_BT_STATUS_NON_CONNECTED_IDLE == pCoexDm->btStatus) ||
		(BT_8723B_1ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus) )
	{
		halbtc8723b1ant_ActionBtConnectedOrNoConnectedIdle(pBtCoexist,
...
halbtc8723b1ant_ActionWifiConnectedSpecialPacket(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bUnder4way
	)
{
	u1Byte	drv_in_test = 0;

	// power save must open before psTdma open.
	// but here lots of TDMA dont ChangeRF state, so leave LPS is needed
	//halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_OFF, 0x0, 0x0);
	...
	)
{
	...
	BOOLEAN		bWifiUnderSoftAP = FALSE;
	u4Byte		wifiBw;

	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], CoexForWifiConnect()===>\n"));

	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_AP_MODE_ENABLE, &bWifiUnderSoftAP);
 	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_4_WAY_PROGRESS, &bUnder4way);
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_DHCP, &bUnderDHCP);
	if(bUnder4way || bUnderDHCP)
	{
		halbtc8723b1ant_ActionWifiConnectedSpecialPacket(pBtCoexist, bUnder4way);
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], CoexForWifiConnect(), return for wifi is under 4way/dhcp<===\n"));
		return;
	}
	...
	/* FINISH_SCAN will enter scan again here, so move bScan to halbtc8723b1ant_RunCoexistMechanism */
	...
	else //wifi busy
	{
		...
		if(BT_8723B_1ANT_BT_STATUS_NON_CONNECTED_IDLE == pCoexDm->btStatus)
		{
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], NON_CONNECTED_IDLE is under progress!!!\n"));
			halbtc8723b1ant_ActionBtConnectedOrNoConnectedIdle(pBtCoexist,
				BT_8723B_1ANT_WIFI_STATUS_CONNECTED_BUSY);
		}
		else if(BT_8723B_1ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus)
		{
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], CONNECTED_IDLE is under progress!!!\n"));
...
{
	PBTC_BT_LINK_INFO pBtLinkInfo=&pBtCoexist->btLinkInfo;
	BOOLEAN	bWifiBusy=FALSE, bWifiConnected=FALSE, bBtHsOn=FALSE;
	BOOLEAN	bLimitedDig=FALSE;
	BOOLEAN	bBtCtrlAggBufSize=FALSE;
	u1Byte aggBufSize=5;
	BOOLEAN	 bWifiUnderSoftAP=FALSE, bUnder4way=FALSE, bScan=FALSE;
	...
	if (bScan) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], RunCoexistMechanism(), return for BTC_SCAN_START <===\n"));
		return;
	}
	...
	if(bWifiUnderSoftAP) {
		if(pBtCoexist->btInfo.bBtDisabled) {
			halbtc8723b1ant_PowerSaveforSoftAP(pBtCoexist, 0);
			if(pCoexDm->curPsTdma != 40) {
				halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, FALSE, 9);
				halbtc8723b1ant_CoexTableWithType(pBtCoexist, NORMAL_EXEC, 0);
			}
			return;
		} else if(!GLBtCoexist.btInfo.bBtBusy){
			halbtc8723b1ant_PowerSaveforSoftAP(pBtCoexist, 0);
			if(pCoexDm->curPsTdma == 40)
				return;
		}
		...
		pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_BUSY, &bWifiBusy);
		if(bWifiBusy ||!bWifiConnected || bUnder4way)
		{
			halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_OFF, 0x0, 0x0);
			halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, FALSE, 9);
		}
	...
	if(!bWifiConnected && !bWifiUnderSoftAP)
	{
		BOOLEAN	bScan=FALSE, bLink=FALSE, bRoam=FALSE;
	...
	)
{
	...
	pBtCoexist->fBtcSetRfReg(pBtCoexist, BTC_RF_A, 0x2, 0xfffff, 0x780); //BT goto standby while GNT_BT 1-->0
	...
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_U4_WIFI_OUT_DPDT, &OutDpdt);
	BTC_PRINT(BTC_MSG_INTERFACE, INTF_INIT, ("[BTCoex], WiFi fw ver:%x!!\n",fwVer));

	if (fwVer < 0xc00 || OutDpdt)  //ext switch for fw ver < 0xc
	{
		/* Internal switch: */
		/* 0x948[11:0] = 0x0 (WL fixed at S1, BT fixed at S0) */
		pBtCoexist->fBtcWrite2Byte(pBtCoexist, 0x948, 0x00);
		/* 0x67[5] = 1 for WL on (S1/S0 control by WL 0x948) */
		pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x67, 0x20, 0x1);

		/* External switch: */
		/* 0x38[11] = 0x1(WL on), Efuse 0x5[3] =1 (WL off, EECS Mux) */
		u4Tmp = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x38);
		u4Tmp |= BIT11;
		...
		if(pBoardInfo->btdmAntPos == BTC_ANTENNA_AT_MAIN_PORT)
		{
	 		pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x92c, 0x3, 0x2); //Ant on Spred phone default to BT because no link no scan
			pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x64, 0x1, 0x1); //Main Ant to  BT for IPS case 0x4c[23]=1

			//tell firmware "no antenna inverse"
			BTC_PRINT(BTC_MSG_INTERFACE, INTF_INIT, ("[BTCoex], no antenna inverse.\n"));
			H2C_Parameter[0] = 0;
		...
		else
		{
			...
			BTC_PRINT(BTC_MSG_INTERFACE, INTF_INIT, ("[BTCoex], antenna inverse.\n"));
			H2C_Parameter[0] = 1;
			H2C_Parameter[1] = 1;  //ext switch type
			pBtCoexist->fBtcFillH2c(pBtCoexist, 0x65, 2, H2C_Parameter);
		}
	...
	{
		/* Internal switch: (WL/BT S1/S0 switch) */
		/* 0x67[5] = 1 for WL on (S1/S0 control by WL 0x948) */
		pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x67, 0x20, 0x1);

		/* External switch: (switch fixed at S1) */
		/* 0x4c[23]=1, 0x4c[24]=0  S0/S1 control by 0x64 */
		u4Tmp = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x4c);
		u4Tmp |= BIT23;
		u4Tmp &=~BIT24;
		pBtCoexist->fBtcWrite4Byte(pBtCoexist, 0x4c, u4Tmp);
		/* 0x64[0] = 0 (WL on->off, Ant->S1 control by WL not power off */
		pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x64, 0x1, 0x0);
		/* 0x38[11] = 0x1(WiFi on), Efuse 0x5[3] =1 (WL off, EECS Mux) */
		u4Tmp = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x38);
		u4Tmp |= BIT11;
		pBtCoexist->fBtcWrite4Byte(pBtCoexist, 0x38, u4Tmp);
		/* 0x92c[1:0] = 01, 0x930[7:0] = 0x77 (WL on, Ant fixed at S1) */
		//pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x92c, 0x3, 0x1);

		if(pBoardInfo->btdmAntPos == BTC_ANTENNA_AT_MAIN_PORT)
		{
			pBtCoexist->fBtcWrite2Byte(pBtCoexist, 0x948, 0x200);//PTA
	...
	)
{
	...
	if(BTC_IPS_ENTER == type)
	{
		...
		if (fwVer < 0xc00 || OutDpdt)  //ext switch
		{
			/* Internal switch: */
			...
			/* External switch */
			/* 0x64[0] = 1 (WL on->off, Ant->S0) */
			pBtCoexist->fBtcWrite1ByteBitMask(pBtCoexist, 0x64, 0x1, 0x1);
			/* 0x4c[24:23] = 00 (WL off, ext. switch control by BT vendor reg 0xac[15:0])*/
			// 0x4c[23-24]=0, Set Antenna control by BT_RFE_CTRL
			u4Tmp = pBtCoexist->fBtcRead4Byte(pBtCoexist, 0x4c);
			u4Tmp &= ~BIT24;
			u4Tmp &= ~BIT23;
			pBtCoexist->fBtcWrite4Byte(pBtCoexist, 0x4c, u4Tmp);
	...
	)
{
	...
	{
		BTC_PRINT(BTC_MSG_INTERFACE, INTF_NOTIFY, ("[BTCoex], SCAN START notify bWifiConnected:%d\n",bWifiConnected));
		...
		{
			halbtc8723b1ant_ActionWifiConnectedScan(pBtCoexist);
			halbtc8723b1ant_SetPsScan(pBtCoexist, TRUE);
		}

		/* because scan_compensation happen after all scan actions in translate scan */
		/* and all TDMA have changed in that time, so here we should remeber if we should */
		/* scan_compensation later in translate scan */
		pBtCoexist->fBtcSet(pBtCoexist, BTC_SET_UI_SCAN_SIG_COMPENSATION, &scan_compensation);
		if((BT_8723B_1ANT_BT_STATUS_SCO_BUSY == pCoexDm->btStatus) ||
			(BT_8723B_1ANT_BT_STATUS_ACL_SCO_BUSY == pCoexDm->btStatus)) {
			scan_compensation = 15;
			pBtCoexist->fBtcSet(pBtCoexist, BTC_SET_UI_SCAN_SIG_COMPENSATION, &scan_compensation);
		}
	...
	else if(BTC_SCAN_FINISH == type)
	{
		...
		{
			halbtc8723b1ant_SetPsScan(pBtCoexist, FALSE);
			halbtc8723b1ant_ActionWifiConnected(pBtCoexist);
		}
	}
}
	...
	)
{
	...
	/* if WIFI busy, TDMA is ok for Tx/Rx, So we dont need to change it */
	pBtCoexist->fBtcGet(pBtCoexist, BTC_GET_BL_WIFI_BUSY, &bWifiBusy);
	if(bWifiBusy)
		return;
		...
		BTC_PACKET_EAPOL == type )
	{
		...
		halbtc8723b1ant_ActionWifiConnectedSpecialPacket(pBtCoexist, FALSE);
	}
}
	...
	)
{
	...
	halbtc8723b1ant_QueryBtInfo(pBtCoexist);
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], ==========================Periodical End=======================\n"));
}

VOID
EXhalbtc8723b1ant_SignalCompensation(
	IN	PBTC_COEXIST pBtCoexist,
	IN	u8 *rssi_wifi,
	IN	u8 *rssi_bt
	)
...
//	RTPRINT(FBT, BT_TRACE, ("[BTCoex], %s\n", __FUNCTION__));
	//pHalData = GET_HAL_DATA(pBtCoexist->Adapter);
	...
	{
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], 1AntSgnlCmpnstn, case %d, WiFiCmpnstn=%d(%d => %d)\n",
				pCoexDm->curPsTdma, RSSI_WiFi_Cmpnstn, *rssi_wifi, *rssi_wifi+RSSI_WiFi_Cmpnstn));
		*rssi_wifi += RSSI_WiFi_Cmpnstn;
	}

	if (rssi_bt && RSSI_BT_Cmpnstn)
	{
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE, ("[BTCoex], 1AntSgnlCmpnstn, case %d, BTCmpnstn=%d(%d => %d)\n",
				pCoexDm->curPsTdma, RSSI_BT_Cmpnstn, *rssi_bt, *rssi_bt+RSSI_BT_Cmpnstn));
		*rssi_bt += RSSI_BT_Cmpnstn;
	}
}

VOID
EXhalbtc8723b1ant_LpsLeave(
	IN	PBTC_COEXIST pBtCoexist
)
{
	halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_OFF, 0x0, 0x0);
...
VOID
EXhalbtc8723b1ant_LowWiFiTrafficNotify(
	IN	PBTC_COEXIST pBtCoexist
)
{
	BOOLEAN bWifiConnected=FALSE;
	...
	if (lpsMode && !pCoexDm->bCurPsTdmaOn) {
		RTPRINT(FBT, BT_TRACE, ("\n[BTCoex], 1Ant for LowWiFiTraffic\n"));
		halbtc8723b1ant_PowerSaveState(pBtCoexist, BTC_PS_LPS_ON, 0x0, 0x0);
		halbtc8723b1ant_PsTdma(pBtCoexist, NORMAL_EXEC, TRUE, 18);
	}
}
#endif
