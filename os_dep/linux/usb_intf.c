...
#define RTL8723B_USB_IDS \
	/*=== Realtek demoboard ===*/ \
	{USB_DEVICE(USB_VENDER_ID_REALTEK, 0xB723)},/* Default ID */
...
static struct usb_device_id rtl8723b_usb_id_tbl[] ={
	RTL8723B_USB_IDS
	{}	/* Terminating entry */
};

struct rtw_usb_drv rtl8723b_usb_drv = {
	.usbdrv.name = (char*)"rtl8723bu",
	.usbdrv.probe = rtw_drv_init,
	.usbdrv.disconnect = rtw_dev_remove,
	.usbdrv.id_table = rtl8723b_usb_id_tbl,
	.usbdrv.suspend =  rtw_suspend,
	.usbdrv.resume = rtw_resume,
	#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 22))
  	.usbdrv.reset_resume   = rtw_resume,
...
static struct rtw_usb_drv *usb_drv = &rtl8723b_usb_drv;
...
static void rtw_dev_unload(_adapter *padapter)
{
	...
	if(padapter->bup == _TRUE)
	{
		...
		if(padapter->bSurpriseRemoved == _FALSE)
		{
			...
			#ifdef CONFIG_WOWLAN
			if(adapter_to_pwrctl(padapter)->bSupportWakeOnWlan==_TRUE){
...
#ifdef CONFIG_WOWLAN
	pwrpriv->bSupportWakeOnWlan=_TRUE;
#else
	//s2.
	//s2-1.  issue rtw_disassoc_cmd to fw
	disconnect_hdl(padapter, NULL);
...
#ifdef CONFIG_BT_COEXIST
	if(1 == adapter_to_pwrctl(if1)->autopm_cnt){
		#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,33))
		...
		#endif
		adapter_to_pwrctl(if1)->autopm_cnt --;
	}
#endif
